...
\begin{itemize}
    \item \textbf{Preprocesamiento:} Conversión de imágenes a escala de grises para reducir la carga computacional y resaltar formas; y uso de la librería dlib para detectar 68 puntos de referencia faciales y recortar el área del rostro (rostro completo y mitad inferior para simular oclusión) \cite{shomoye2024}.
    \item \textbf{Arquitectura:} Se modificó ResNet50 descongelando los bloques 4 y 5 para un ajuste fino (\textit{fine-tuning}). Se añadieron capas de Dropout (0.5 y 0.6) y regularización L2 para controlar el sobreajuste (\textit{overfitting}) \cite{shomoye2024}.
    \item \textbf{Entrenamiento:} Se utilizaron técnicas de aumento de datos (giros y rotaciones aleatorias), el optimizador Adam y una programación de la tasa de aprendizaje (\textit{ReduceLROnPlateau}) \cite{shomoye2024}.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Superioridad de ResNet50:} En comparaciones iniciales, ResNet50 demostró un rendimiento superior en precisión y generalización frente a VGG19 y MobileNet para la detección de emociones en RV \cite{shomoye2024}.
    \item \textbf{Eficacia del Modelo Personalizado:} El modelo ajustado superó sistemáticamente al ResNet50 predeterminado en todos los experimentos, mostrando una mayor robustez ante datos nuevos y ocluidos \cite{shomoye2024}.
    \item \textbf{Detección de Aburrimiento:} Se logró diferenciar exitosamente el aburrimiento de la neutralidad, obteniendo resultados razonables incluso con la cara parcialmente cubierta \cite{shomoye2024}.
    \item \textbf{Eficiencia Técnica:} Se confirmó que el preprocesamiento en escala de grises optimiza el entrenamiento al reducir canales de datos sin perder características esenciales de las expresiones \cite{shomoye2024}.
\end{itemize}
